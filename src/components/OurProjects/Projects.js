...
const fireVideo  = (e,n,onfun,offun) =>{
  if(e === n ){return onfun} else {return offun}
}    
useEffect(() => {
  console.log(index)
  fireVideo()
    ...
    const dataProjects = [
      ...
      {video:factory,head:projectsLangHead[4],paragraph:projectsLangPara[0]},
    ]

    return  (<div className='projects  py-5'>
        <div className=''>
        <h2  className='head-section  text-center py-3 mb-5'>{projectsLang.header}</h2>
        <Slider {...settings}>

        {!dataProjects ? " " : dataProjects.map((el,n)=>{
          return    <div className='p-2'>
                    <div class="card project-card" >
                    <div className='media-overlay'>
                    {!el.video
                     ? <img src="https://via.placeholder.com/150" class="card-img-top" alt="" height={"100%"} />

                     :<div className='video-overlay position-relative h-100 w-100' >
                      <span  className={'position-absolute  bottom-0 start-20 text-light h1 text-center'}>{<AiFillPlaySquare />}</span>
                      <video  type="video/mp4" src={el.video} alt={el.video} autoPlay muted infinite onMouseOver={(e)=>{e.target.play()}}  loop onMouseOut={(e)=>{e.target.pause()}} height={"100%"} />
                      </div>
                    }
                    </div>
